...
/*******************************
  속이 채워진 삼각형을 그린다.
********************************/
void DTRIANGLE :: DrawFill( INT X1, INT Y1, INT X2, INT Y2, INT X3, INT Y3, COLOR Color )
{
    int i;
    FIXED d12, d13, d23;
    FIXED dl, dr;
    ...
    if( Y1 == Y3 ){
      ...
      DLINE::DrawHoriz( X1, Y1, (X3 - X1) + 1, Color );
      return;
    }
    ...
    if( Y1 != Y2 ){
      ...
      for( i = Y1; i <= Y2; i ++ ){
        DLINE::DrawHoriz( FIXEDTOINT(xl), i, FIXEDTOINT(xr - xl) + 1, Color );
        xl += dl;
        xr += dr;
      }
    } else {
...
/*******************************
  속이 채워진 삼각형을 그린다.
********************************/
void DTRIANGLE :: DrawFill( void )
{
    DrawFill( X1, Y1, X2, Y2, X3, Y3, Color );
}
