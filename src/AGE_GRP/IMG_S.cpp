...
/******************************
  이미지의 그림자를 출력한다.
*******************************/
void IMAGE :: PutShadow( INT X, INT Y )
{
...
/*****************************
  이미지를 늘려서 출력한다.
******************************/
void IMAGE :: PutShadowStretch( INT X, INT Y, INT tWidth, INT tHeight )
{
    int i, j, t;
    ...
    for( i = 0; i < tHeight; i ++ ){
      ...
      xx = 0;
      for( j = 0; j < tWidth; j ++ ){
        if( *(sa + FIXEDTOINT(xx)) != tc ) *ScrPtr = pixel;
        ScrPtr ++;
        xx += dx;
      }
...
/**************************************
  이미지를 일정 배율 늘려서 출력한다.
***************************************/
void IMAGE :: PutShadowScale( INT X, INT Y, FIXED Scale )
{
