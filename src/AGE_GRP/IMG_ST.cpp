...
******************************************/
void IMAGE :: PutShadowTrans( INT X, INT Y, COLOR *TransTable )
{
...
/*****************************
  이미지를 늘려서 출력한다.
******************************/
void IMAGE :: PutShadowTransStretch( INT X, INT Y, INT tWidth, INT tHeight, COLOR *TransTable )
{
    int i, j, t;
    ...
    for( i = 0; i < tHeight; i ++ ){
      ...
      for( j = 0; j < tWidth; j ++ ){
        ...
        if( pixel != tc ) *ScrPtr = TransTable[ pixel ];
        ScrPtr ++;
        xx += dx;
      }
...
/**************************************
  이미지를 일정 배율 늘려서 출력한다.
***************************************/
void IMAGE :: PutShadowTransScale( INT X, INT Y, FIXED Scale, COLOR *TransTable )
{
    ...
    PutShadowTransStretch( X, Y, w, h, TransTable );
}
